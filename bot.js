...
async function startBot() {
  ...
  if (mainTrade) {
    ...
    // Use configured initial side or implement your own logic
...
async function monitorPrice() {
  while (state.isRunning()) {
    ...
    // 🟢 STRICT hedge re-entry only on boundary cross and no cooldown
    if (!state.getMainTrade() && !state.getHedgeTrade() && !hedgeOpeningInProgress && !inCooldown) {
...
async function openHedgeTrade(side, entryPrice) {
  ...
  // Boundary validation
  if (side === 'Buy' && (!boundaries.bottom || entryPrice > boundaries.bottom)) {
    sendMessage(`⚠️ Cannot open Buy hedge above bottom boundary`);
    return;
  }
  if (side === 'Sell' && (!boundaries.top || entryPrice < boundaries.top)) {
    sendMessage(`⚠️ Cannot open Sell hedge below top boundary`);
    return;
  }

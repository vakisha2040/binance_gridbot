...
class BitgetClient {
  ...
  async setLeverage(symbol, leverage, holdSide = 'long') {
    try {
      ...
      this.logger.info(`✅ Leverage set to ${leverage}x (${this.config.marginMode}) for ${symbol} (${holdSide})`);
      this.sendMessage?.(`✅ Leverage set to ${leverage}x (${this.config.marginMode}) for ${symbol} (${holdSide})`);
      return true;
    } catch (e) {
